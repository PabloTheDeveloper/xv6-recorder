...
{
	// some variables to pass to functions (should match with others)
	...
	if(all || test == 0){
		printf(1, "ONE REPLAY OF EACH SYSCALL:\n\n");
	...
	if(all || test == 1){
		printf(1, "\n\nREPLAYING ONLY 1000 EACH SYSCALL:\n\n");
		const char * scs[] = {
			"/k_getpid",
			"/k_uptime",
			"/k_open",
	...
	if(all || test == 2){
		printf(1, "\n\nWRITING 1001 replay OF EACH SYSCALL(should fail each):\n\n");
		const char * scs[] = {
			"k1_getpid",
			"k1_uptime",
			"k1_open",
		...
		for (int i = 0; i < 11; i++){
			printf(1, "TEST MSG: should throw file not found error down below\n");
			if(fork() == 0){
				printf(1, "replaying %s\n", scs[i]);
				replay_start(scs[i]);
